<template>
  <div class="mx-5">
    <div style="display:flex; align-items:center" class="text-subtitle-2 my-2">
      ...
      <div style="margin-left:auto">
        <div style="font-weight:700">
          ...
          <span class="mx-5">
            <v-icon color='green' small class="mb-1 mx-1">
              {{'mdi-checkbox-marked-circle'}}
            </v-icon>
            <span>運轉中</span>
          </span>
          <span class="mx-5">
            <v-icon color='yellow' small class="mb-1 mx-1">
              {{'mdi-stop-circle'}}
            </v-icon>
            <span>待機中</span>
          </span>
          <span class="mx-5">
            <v-icon color='red' small class="mb-1 mx-1">
              {{'mdi-close-octagon'}}
            </v-icon>
            <span>停機中</span>
          </span>
...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    data: () => ({
      factoryDataTitle: ['案場名稱', '建置容量(kWh)', '饋線容量(kW)​', 'SOC(%)', 'SOH(%)​', '充放電(+/-kW)​', '轉換效率(%)​', '直流功率(+/-kW)', '排程運轉中​', '案場狀態']
    }),
    methods: {
      toInfoPage () {
        const params = {
